...
module.exports = function (data, opts) {
	mkdirp("flat_files/individuals", function() {
		...
		if (opts.format === "csv" || opts.format === "csvs") {
			...
			data.forEach(function(name) {
				...
				if (opts.format === "csv") {
					console.log("Writing", row.id);
					console.log(headers.map(d => row[d]).join(",") + "\n");
		...
		if (/json/.test(opts.format)) {
			...
			fs.writeFileSync("./flat_files/roster.json", JSON.stringify(roster));
		}
	});
}